...
// ConcourseTeamSpec defines the desired state of ConcourseTeam.
type ConcourseTeamSpec struct {
	// InstanceRef references the ConcourseInstance this team belongs to.
...
// ConcourseTeamStatus defines the observed state of ConcourseTeam.
type ConcourseTeamStatus struct {
	// +listType=map
	// +listMapKey=type
	// +optional
...
// ConcourseTeam manages a Concourse team and its auth configuration.
type ConcourseTeam struct {
	...
	// +required
	Spec ConcourseTeamSpec `json:"spec"`

	// +optional
	Status ConcourseTeamStatus `json:"status,omitzero"`
}

// +kubebuilder:object:root=true

// ConcourseTeamList contains a list of ConcourseTeam.
type ConcourseTeamList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitzero"`
	Items           []ConcourseTeam `json:"items"`
}

func init() {
	SchemeBuilder.Register(&ConcourseTeam{}, &ConcourseTeamList{})
}
