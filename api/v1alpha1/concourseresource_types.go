...
// ConcourseResourceSpec defines the desired state of ConcourseResource.
type ConcourseResourceSpec struct {
	// PipelineRef references the ConcoursePipeline that owns this resource.
...
// ConcourseResourceStatus defines the observed state of ConcourseResource.
type ConcourseResourceStatus struct {
	// +listType=map
	// +listMapKey=type
	// +optional
...
// ConcourseResource manages a resource within a Concourse pipeline (pin, check).
type ConcourseResource struct {
	...
	// +required
	Spec ConcourseResourceSpec `json:"spec"`

	// +optional
	Status ConcourseResourceStatus `json:"status,omitzero"`
}

// +kubebuilder:object:root=true

// ConcourseResourceList contains a list of ConcourseResource.
type ConcourseResourceList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitzero"`
	Items           []ConcourseResource `json:"items"`
}

func init() {
	SchemeBuilder.Register(&ConcourseResource{}, &ConcourseResourceList{})
}
