...
| `triggerBuild` | boolean | no | `false` | If `true`, trigger a build on the next reconcile and create a `Build` CR. |
...
| `nextBuildName` | string | Name of the `Build` CR created by the last `triggerBuild`. |
...
- `triggerBuild: true` is a one-shot signal: once the build is triggered and a `Build` CR is created, you should set `triggerBuild: false` to avoid triggering on every subsequent reconcile. The operator does not automatically reset this field.
- The created `Build` CR name is stored in `status.nextBuildName`.
