...
var _ = Describe("ConcourseInstance Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind ConcourseInstance")
			err := k8sClient.Get(ctx, typeNamespacedName, instance)
			if err != nil && errors.IsNotFound(err) {
				resource := &concoursev1alpha1.ConcourseInstance{
					ObjectMeta: metav1.ObjectMeta{
						Name:      resourceName,
						Namespace: "default",
					},
					Spec: concoursev1alpha1.ConcourseInstanceSpec{
		...
		AfterEach(func() {
			...
			if err == nil {
				By("Cleanup the specific resource instance ConcourseInstance")
		...
		It("should successfully reconcile the resource without error", func() {
			...
			reconciler := &ConcourseInstanceReconciler{
				Client: k8sClient,
		...
		It("should add the instance finalizer on first reconcile", func() {
			...
			finalizerInst := &concoursev1alpha1.ConcourseInstance{
			...
			DeferCleanup(func() {
				...
				latest := &concoursev1alpha1.ConcourseInstance{}
			...
			fetched := &concoursev1alpha1.ConcourseInstance{}
		...
		It("should set Authenticated=False and Ready=False when Concourse is unreachable", func() {
			By("Creating a fresh instance for this test")
			authInst := &concoursev1alpha1.ConcourseInstance{
	...
	Context("When deleting a resource", func() {
		...
		It("should remove the finalizer and not error on deletion", func() {
			...
			afterDeletion := &concoursev1alpha1.ConcourseInstance{}
