...
var _ = Describe("ConcourseResource Controller", func() {
	Context("When reconciling a resource", func() {
		const resourceName = "test-resource"

		ctx := context.Background()
		typeNamespacedName := types.NamespacedName{Name: resourceName, Namespace: "default"}
		concourseresource := &concoursev1alpha1.ConcourseResource{}

		BeforeEach(func() {
			By("creating the custom resource for the Kind ConcourseResource")
		...
		AfterEach(func() {
			...
			if err == nil {
				By("Cleanup the specific resource instance ConcourseResource")
