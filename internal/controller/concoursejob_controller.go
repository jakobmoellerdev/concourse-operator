...
// ConcourseJobReconciler reconciles a ConcourseJob object.
type ConcourseJobReconciler struct {
	client.Client
	Scheme *runtime.Scheme
	Cache  *concourse.Cache
}

// +kubebuilder:rbac:groups=concourse.concourse-ci.org,resources=concoursejobs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=concourse.concourse-ci.org,resources=concoursejobs/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=concourse.concourse-ci.org,resources=concoursejobs/finalizers,verbs=update
...
// +kubebuilder:rbac:groups=concourse.concourse-ci.org,resources=concoursebuilds,verbs=get;list;watch;create;update;patch

func (r *ConcourseJobReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if job.Spec.TriggerBuild && job.Status.ObservedGeneration != job.Generation {
		buildCR := &concoursev1alpha1.ConcourseBuild{
...
func (r *ConcourseJobReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&concoursev1alpha1.ConcourseJob{}).
		Named("concoursejob").
		Complete(r)
}
